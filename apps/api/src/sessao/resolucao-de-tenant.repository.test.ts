...
describe('ResolucaoDeTenantRepository: toda operação sem escopo é marcada e justificada', () => {
  it('nasce só com os métodos da tabela da seção 6 que a tarefa usa, cada um com @SemEscopo e a justificativa dele', () => {
    expect(metodosDe(ResolucaoDeTenantRepository)).toEqual(['contaPorEmail', 'criarContas', 'gravarFalhaDeLoginPorEmail', 'sessaoParaRenovar', 'usuariosAtivosDaConta'])
  ...
  it('a criação de usuário e sessão, o registro de acesso, o /v1/eu e as escritas da renovação, da atividade e da saída não saem sem escopo: usam a escola do contexto', () => {
    for (const classe of [CriacaoDeSessaoRepository, RegistroDeAcessoRepository, EuRepository, EscritaDeSessaoRepository]) {
