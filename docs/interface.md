...
**Parte B** (seções 8 a 12) é o design. A **direção** da Parte B foi dada pelo Gabriel em
19/09/2026 — a estrutura do chat do Mulerun, a marca da landing page `turmma.com`, e
componente vindo do 21st.dev, nada genérico. O **detalhamento** é proposta, a revisar por ele;
aprovado, vira decisão por `/registrar-decisao` e sai da seção 7.
...
O desenho visual desta casca, com medidas e componentes, está na seção 11.
...
O professor pode escolher a ferramenta no próprio chat (*proposta:* um seletor dentro da caixa
de pedido, os atalhos de ferramenta logo abaixo dela, e atalho digitando `/` — desenho na
seção 11.2).
...
- **Trazer para `apps/web`** os tokens da seção 9.9, as duas fontes, os SVGs da marca e o
  avatar dos seis agentes — é a primeira tarefa de tela da A1 (D71). Hoje a casca do F0 ainda
  usa a paleta `slate` e a fonte do sistema
- **Avatar dos seis agentes**: há proposta na seção 9.7 (ícone de função sobre ladrilho na cor
  da rampa da marca); falta o Gabriel fechar ícone e cor de cada um. A landing page ainda lista
  o "Monitor de turma" e não tem o Assistente de ensino (D32 revista): alinhar junto
...
- **Vestir a família Agent Elements**: 121 classes de cor fixa e 63 variantes `dark:` trocadas à
  mão nas sete peças do chat (10.2). Entra na estimativa da A2
...
**Direção dada** (Gabriel, 19/09/2026): três fontes, cada uma com um papel.

| Fonte | O que ela dá | O que ela não dá |
|---|---|---|
| **Mulerun** (`mulerun.com/pt-BR/chat`) | A **estrutura**: lateral recolhível, Home que é uma caixa de pedido no centro, atalhos logo abaixo, histórico na lateral, tela de entrada dividida | Cor, fonte, tom e qualquer coisa de modelo de IA |
| **Landing page** (`turmma.com`, `~/Code/turmma-lp`) e **manual da marca** (`~/Code/turmma-marca`) | A **pele**: paleta, as duas fontes, o botão caramelo, as durações e curvas de movimento, a pinta, o monograma dos agentes | Layout de página de venda: bloco de cor de tela inteira, manchete gigante, entrada animada por seção |
| **21st.dev** | As **peças**: todo componente de interface sai do catálogo (seção 10), adaptado aos nossos tokens | Decisão de produto. O catálogo tem coisa que aqui é proibida por lei (seção 10.3) |

### 8.2 O que pegamos do Mulerun

Medido na tela pública em 19/09/2026:

- **Duas regiões.** Lateral de **260 px** num tom um degrau abaixo do fundo, sem borda pesada;
  conteúdo no fundo claro. A lateral recolhe por um botão no topo dela
- **Anatomia da lateral**, de cima para baixo: marca e botão de recolher · ação primária em
  caixa com borda ("Nova Tarefa", com o atalho à direita) · busca · itens de navegação com
  ícone e rótulo, em linhas de **36 px** com canto de **8 px** · divisor · histórico recolhível
  com estado vazio próprio · rodapé
- **Home = uma pergunta.** Coluna central de **~760 px**: uma pílula pequena de novidade, a
  saudação em duas linhas (a primeira forte, a segunda rebaixada), a caixa de pedido grande com
  canto de **24 px**, borda de 1 px e quase nenhuma sombra, e uma fileira de atalhos com ícone
- **A caixa de pedido tem barra própria**: à esquerda o que se acrescenta ao pedido (anexo,
  ferramentas), à direita o seletor de contexto e o botão redondo de enviar
- **Quase sem sombra.** A hierarquia vem de tom de fundo e de linha fina, não de elevação
- **Entrada dividida**: ilustração de um lado, formulário do outro
...
- **Cinza neutro frio e cor em `oklch()`**. O chão aqui é quente (9.1), e `oklch()` não existe
  no Chrome 109 do laboratório: o e2e já reprova `oklch()` no CSS servido
...
2. **Quente no chão, frio na decisão.** Papel, creme e caramelo fazem o chão e a ação de
   criar. O **azul-noite fica reservado para a decisão oficial**: aprovar, publicar, confirmar.
   O botão de aprovar não se parece com nenhum outro botão do produto, e isso é de propósito:
   é o que combate o clique reflexo (regra 50, item 8)
...
As quatro cores do manual (D54) — caramelo `#E8732E`, azul-noite `#16233E`, creme `#FFF3E2`,
papel `#FDFBF7` — mais o que a landing page já derivou delas e o que a interface precisa. Tudo
em **hex**. Contraste medido (WCAG 2.1) entre parênteses.

**Chão e estrutura** — quentes, acompanham o papel:

| Token | Hex | Uso |
|---|---|---|
| `fundo` | `#FDFBF7` | Fundo do app. É o papel do manual: branco puro cansa em sala clara |
| `superficie` | `#FFFFFF` | Cartão, caixa de pedido, campo, menu |
| `lateral` | `#F4F0E8` | Fundo da lateral |
| `realce` | `#E6E0D4` | Hover e selecionado: item da lateral, item de menu, linha de tabela. Dá 1,16:1 contra a lateral e 1,31:1 contra o branco — perceptível, mas discreto, então **selecionado nunca é só o fundo** (11.1) |
| `linha` | `#E6E1D8` | Divisor e borda de cartão. Decorativa: nunca é a única pista de um controle |
| `borda-campo` | `#857E70` | Borda de campo e de botão secundário (4,0:1 no branco, 3,5:1 na lateral — passa o 3:1 de componente) |

**Texto:**

| Token | Hex | Uso |
|---|---|---|
| `tinta` | `#0A1020` | Texto principal (18,3:1 no fundo; 16,7:1 na lateral) |
| `apoio` | `#3F4859` | Texto de apoio (8,9:1 no fundo; 8,1:1 na lateral) |
| `sutil` | `#5B6475` | Placeholder, metadado, segunda linha da saudação (5,8:1 no fundo; 5,2:1 na lateral; 4,5:1 no realce, no limite). É o piso: nada de texto mais claro que isto, e sobre `realce` prefira `apoio` (7,0:1) |
| `inativo` | `#8A8F9A` | Só controle desabilitado (3,2:1) |

**Marca:**

| Token | Hex | Uso |
|---|---|---|
| `caramelo` | `#E8732E` | Preenchimento da ação primária **com texto `tinta` em cima** (6,3:1), a pinta, o ladrilho do Tutor |
| `caramelo-claro` | `#F08A48` | Hover da ação primária (7,6:1 com `tinta`) |
| `caramelo-fundo` | `#D4651F` | Pressionado (5,1:1 com `tinta`) |
| `caramelo-texto` | `#A85220` | Caramelo **como texto** e link (5,4:1 no branco; 4,9:1 no creme) |
| `noite` | `#16233E` | Ação oficial, painel da entrada, dica flutuante, anel de foco (branco em cima: 15,6:1) |
| `noite-alto` | `#22314F` | Hover da ação oficial (13,0:1) |
| `creme` | `#FFF3E2` | Bolha de quem escreve e bloco de respiro. Não serve de hover: sobre o branco dá 1,1:1 e some em tela fraca |

Duas proibições que a landing page já pagou para aprender: **caramelo nunca é texto abaixo de
24 px** sobre claro (2,9:1 no papel) e **branco nunca vai em cima do caramelo** (3,0:1). Para isso
existem `caramelo-texto` e o par caramelo + tinta.

**Estado** — cada família é um fundo e um texto:

| Família | Fundo | Texto | Quando |
|---|---|---|---|
| `pendente` | `#FDF0E8` | `#7A3A08` (7,7:1) | "Esperando você". Nesta marca pendência é caramelo, não âmbar |
| `ok` | `#E6F1EC` | `#0B5740` (7,4:1) | Aprovado, conferido, salvo. É a segunda voz do produto: metade do argumento é "um humano aprovou" |
| `erro` | `#F9EDEB` | `#A32E20` (6,2:1) | Erro, rejeição, ação destrutiva |
| `info` | `#E9EDF4` | `#16233E` (13,3:1) | Aviso permanente (o professor acompanha o Tutor), explicação de autonomia |
| `ia` | `#F1EDE5` | `#3F4859` (7,9:1) | O selo "IA" e o chip de fonte. Neutro de propósito: o selo informa, não chama |

Estado nunca é só cor: vem sempre com texto ou ícone (regra 50, item 11). Só tema claro.

### 9.2 Tipografia

As duas fontes da landing page, **servidas pelo próprio app** em `woff2` com subconjunto latino:
**Fustat 600** (12 KB) nos títulos e **Inter variável 400–700** (48 KB) em todo o resto.
`font-display: swap`, `preload` das duas, e `system-ui` como reserva. 60 KB no total é o teto:
não entra terceira fonte nem outro peso de Fustat.

| Papel | Fonte | Tamanho / entrelinha | Onde |
|---|---|---|---|
| Saudação | Fustat 600 | `clamp(28px, 4vw, 36px)` / 1,1 · espaçamento −0,035em | Home |
| Título de tela | Fustat 600 | 22 / 1,2 · −0,02em | Topo de cada seção |
| Título de cartão | Inter 600 | 16 / 1,35 | Cartão, thread, diálogo |
| Número de painel | Fustat 600 | 28 / 1 · `tabular-nums` | Governança, Meu painel |
| Corpo | Inter 400 | 16 / 1,55 | Conversa, formulário, texto do aluno |
| Interface densa | Inter 400–500 | 14 / 1,45 | Tabela, metadado, item de menu secundário |
| Rótulo | Inter 600 | 12 / 1 · caixa-alta · +0,08em | Cabeçalho de grupo da lateral, cabeçalho de tabela |
...
- **Canto:** 8 px em linha de menu, chip e selo · 12 px em botão e campo · 16 px em cartão e
  bolha · 24 px na caixa de pedido e em diálogo · círculo só em botão de enviar e contador
- **Espaço:** grade de 4 px. Respiro interno de cartão 16 px (20 px a partir de 1024 px).
  Coluna de conversa e de Home com até 760 px; tela de tabela com até 1180 px
...
- **Elevação:** o padrão é **linha fina, sem sombra**. Sombra só no que flutua — menu, dica,
  diálogo, aviso temporário — e é uma só: `0 10px 30px -12px rgba(10,16,32,.18)`. A caixa de
  pedido leva `0 1px 2px rgba(10,16,32,.04)` e mais nada
...
As durações e curvas são as da landing page, para o produto e a página terem a mesma mão:
...
| Entrada | até 640 ms | `cubic-bezier(.22,1,.36,1)` | Saudação da Home, mensagem nova, cartão de ferramenta |
...
4. **A saudação entra uma vez por sessão**, palavra a palavra, e o brilho da pílula da Home dá
   **uma passada** e para
...
- **Pinta + "Turmma" em Fustat** no topo da lateral; recolhida, fica só a pinta. `favicon` e
  ícone de aplicativo são o `turmma-icone.svg` (pinta caramelo no ladrilho azul-noite)
...
- **Cada turma tem a pinta dela** (manual da marca): o sistema gera uma pinta própria, sempre a
  ...
  *Proposta:* entra na A1 como avatar estático por turma do seed, e a geração vem com o F2
...
*Proposta, a fechar pelo Gabriel.* O agente tem identidade de função e **nunca parece uma
pessoa** (D17, D58): sem rosto, sem foto, sem nome próprio, sem mascote. O avatar é um
**ladrilho** com o mesmo canto do ícone da marca (24% do lado), na cor da rampa entre o caramelo
e o azul-noite — nada de arco-íris —, com o **ícone da função** dentro.

| Agente | Ladrilho | Ícone (lucide) | Cor do ícone |
|---|---|---|---|
| Assistente de ensino | `#16233E` | `messages-square` | creme (14,3:1) |
| Tutor | `#E8732E` | `message-circle-question` | tinta (6,3:1) |
| Corretor | `#C2572A` | `clipboard-check` | branco (4,5:1) |
| Planejador | `#243A66` | `calendar-days` | creme (10,2:1) |
| Adaptador | `#D8974A` | `sliders-horizontal` | tinta (7,6:1) |
| Analista de desempenho escolar | `#4C6484` | `chart-column` | branco (6,1:1) |

Tamanhos: 24 px na linha da mensagem, 32 px na lista de threads, 48 px no cabeçalho da thread e
na tela Agentes da coordenação. A cor ajuda, mas o **nome da função aparece sempre ao lado**: o
avatar nunca é a única identificação. O ponto de estado ao lado do avatar diz "trabalhando" ou
"esperando você", nunca "online" — agente não fica "online" como gente.
...
A paleta `slate`/`blue` de hoje dá lugar a esta:
...
@theme {
  ...
  /* chão e estrutura */
  --color-fundo: #FDFBF7;      --color-superficie: #FFFFFF;
  --color-lateral: #F4F0E8;    --color-realce: #E6E0D4;
  --color-linha: #E6E1D8;      --color-borda-campo: #857E70;
  /* texto */
  --color-tinta: #0A1020;      --color-apoio: #3F4859;
  --color-sutil: #5B6475;      --color-inativo: #8A8F9A;
  /* marca */
  --color-caramelo: #E8732E;   --color-caramelo-claro: #F08A48;
  --color-caramelo-fundo: #D4651F;  --color-caramelo-texto: #A85220;
  --color-noite: #16233E;      --color-noite-alto: #22314F;
  --color-creme: #FFF3E2;
  /* estado: fundo e texto */
  --color-pendente-cx: #FDF0E8;  --color-pendente: #7A3A08;
  --color-ok-cx: #E6F1EC;        --color-ok: #0B5740;
  --color-erro-cx: #F9EDEB;      --color-erro: #A32E20;
  --color-info-cx: #E9EDF4;      --color-ia-cx: #F1EDE5;

  --font-titulo: "Fustat", "Inter", system-ui, sans-serif;
  --font-corpo: "Inter", system-ui, sans-serif;

  --radius-linha: 8px;  --radius-controle: 12px;
  --radius-cartao: 16px; --radius-caixa: 24px;

  --t-estado: 150ms; --t-gesto: 260ms; --t-entrada: 640ms;
...
|---|---|---|---|
| `background` | `fundo` | `primary`, `sidebar-primary` | `caramelo` |
| `foreground`, `card-foreground`, `popover-foreground` | `tinta` | `primary-foreground`, `sidebar-primary-foreground` | `tinta` |
| `card`, `popover` | `superficie` | `secondary`, `muted`, `accent` | `realce` |
| `muted-foreground` | `sutil` | `secondary-foreground`, `accent-foreground` | `tinta` |
| `border`, `sidebar-border` | `linha` | `input` | `borda-campo` |
| `ring`, `sidebar-ring` | `noite` | `destructive` · `destructive-foreground` | `erro` · branco (7,1:1) |
| `sidebar` | `lateral` | `sidebar-accent` | `realce` |
| `sidebar-foreground`, `sidebar-accent-foreground` | `tinta` | | |

`accent` é o fundo de hover e de item em foco nos menus do shadcn, e por isso aponta para
`realce`, não para `creme`: creme sobre branco dá 1,1:1 e o hover some em tela fraca.

Peça que usa cor de fábrica direta (`neutral-200`, `zinc-900`, variante `dark:`) **sai sem cor
nenhuma**, porque a paleta é zerada — é assim que a revisão pega peça não vestida. **Foco:**
contorno de 3 px em `noite` com 2 px de afastamento; sobre superfície escura, `#F2A25B` (7,5:1).
Várias peças trazem `outline-none` no campo de texto: sai, para o foco global valer.
...
já têm: `caramelo-claro`, `noite-alto`) ou por `rgba()` literal no fundo de diálogo. Vale pedir
ao e2e que reprove `color-mix(` no CSS servido, como já reprova `oklch(` (Joaquim).
...
| Para | Peça (`autor/slug` · id) | Depende de | Licença | Ao adotar |
|---|---|---|---|---|
| Linha de thread; cartão de entrega (o que fez, o que espera, ações) | `shadcn/item` · 8669 | Radix (slot, avatar, separator, dropdown) | a conferir (o 21st não declara; a origem, shadcn/ui, é MIT) | Semântica. O cartão de entrega usa `shadcn/card` quando precisa de corpo |
...
| Abas de "Meu painel" | `shadcn/tabs` · 953 | Radix | MIT | — |
...
| Painéis | `recharts`, quando existir | Ao abrir Meu painel ou Governança |
| Calendário | `date-fns`, a peça do calendário | Ao abrir Calendário |
| Material | `react-dropzone` | Ao abrir Material (coordenação) |

Fontes: 60 KB, com `preload`. Nenhuma imagem decorativa acima de 30 KB; ilustração da entrada
em SVG ou WebP.
...
│                        │                                                          │
│ [ + Nova conversa    ] │                                                          │
│                        │              conteúdo da seção, em coluna                │
│ ⌂  Home                │        de até 760 px (conversa) ou 1180 px (tabela)      │
│ ▦  Ferramentas         │                                                          │
│ ▤  Calendário          │                                                          │
│ ◈  Seu time        (3) │                                                          │
│ ◔  Meu painel          │                                                          │
...
- **Topo da lateral:** pinta + "Turmma", o botão de recolher, e logo abaixo o **seletor de
  escola** (regra 50, item 13). Para quem tem uma escola só, ele mostra o nome e não abre
- **"Nova conversa"** em caixa com borda, como o "Nova Tarefa" do Mulerun. É ação secundária:
  o caramelo da tela é do botão de enviar
- **Itens** em linhas de 36 px (44 px no celular), canto de 8 px, ícone de 18 px + rótulo.
  Selecionado tem **três pistas, não uma**: fundo `realce`, texto `tinta` em peso 600 e um
  filete de 3 px em `caramelo` na borda esquerda — o fundo sozinho dá 1,16:1 e some na tela de
  um Chromebook de entrada. **"Seu time" leva o contador** de entregas esperando o professor,
  em `pendente`
- **Histórico** recolhível, só o título de cada conversa, com vazio próprio ("Nenhuma conversa
  ainda")
- **Rodapé:** a pessoa, o papel e o menu com Privacidade, Ajuda e **Sair** — a um clique, do
  mesmo tamanho de qualquer outro item (D59)
- **Recolhida**, a lateral vira trilho de 56 px com os ícones e a dica de cada um
- **Botões**, cinco variantes e nenhuma outra:

| Variante | Aparência | Para |
|---|---|---|
| `primario` | fundo `caramelo`, texto `tinta` | **A** ação da tela: enviar, gerar, salvar. No máximo um por tela |
| `oficial` | fundo `noite`, texto branco | **Só decisão oficial**: aprovar, publicar, confirmar envio (8.4, princípio 2) |
| `secundario` | fundo `superficie`, borda `borda-campo`, texto `tinta` | A alternativa, e as duas opções de uma escolha de peso igual |
| `discreto` | sem fundo; hover `realce` | Ação de linha, de barra, de cartão |
| `perigo` | texto `erro`; cheio em `erro` só dentro do diálogo de confirmação | Rejeitar, excluir, revogar |

**O que cada papel vê na lateral:**

| Professor (**decidido**, seção 1) | Aluno (*proposta*) | Coordenação (*proposta*: os onze itens da seção 3 em três grupos e um rodapé) |
|---|---|---|
| Home · Ferramentas · Calendário · Seu time · Meu painel · Histórico | Tutor · Atividades e provas · Meu desempenho · O que o Tutor sabe de mim · rodapé fixo: **Avisar um adulto** e Privacidade | **Escola:** Estrutura, Material, Adaptações · **IA e ensino:** Governança, Analista, Agentes · **Conformidade:** Conformidade, Denúncias, Auditoria, Exportar · rodapé: Configurações |

A coordenação abre em **Governança**, não em chat: é a tela que fecha a venda. O aluno abre no
**Tutor** quando ele está ligado para a turma, e em **Atividades e provas** quando não está.
...
```
              ( ◔ Planejador · Hoje: 3 aulas e 1 entrega em atraso  → )
   ...
   └──────────────────────────────────────────────────────────────────────┘
     [ Prova ]  [ Atividade ]  [ Plano de aula ]  [ Adaptação ]  [ Mais ▾ ]

   ┌ Esperando você ───────────────────────────────────────────────────────┐
   │ ▣ Corretor · 32 provas do 2ºB corrigidas. Abra os 5 destaques      →  │
   └───────────────────────────────────────────────────────────────────────┘
```

- **A pílula é do Planejador.** É a linha com que ele abriu o dia, e leva para "Seu time". É o
  que faz a primeira tela já mostrar o time trabalhando (regra 50, item 6), no lugar onde o
  Mulerun põe anúncio de novidade. Uma só, sem laço de brilho
- **Saudação** pela hora, com o primeiro nome; a segunda linha em `sutil`
- **Caixa de pedido:** à esquerda, o que muda o pedido — **Ferramenta** (D18) e **Web** (D68,
  ...
  lugar onde o Mulerun põe o modelo — e enviar. Digitar `/` abre o mesmo menu de ferramentas.
  *Proposta.* Anexo e busca na web ficam fora do MVP; o lugar deles está reservado
- **Atalhos:** escolher um marca a ferramenta na caixa e põe o cursor nela, com um texto de
  exemplo. **Não envia nada sozinho**
- **"Esperando você"** só existe quando há entrega pendente. Sem pendência, não há cartão — e
  não há enfeite no lugar dele
...
```
  ...
  ┌ Uma pergunta antes ───────────────────────────────────────────────────────────┐
  │ Quer usar a ferramenta Prova? Ela salva na sua biblioteca, liga ao 2ºB e cita │
  │ a página de cada questão.                                                     │
  │                      [ Usar ferramenta Prova ]   [ Só conversar ]             │
  └───────────────────────────────────────────────────────────────────────────────┘
...
- **O selo de IA** (seção 6) é um conjunto só, igual em todo o produto: `[ IA ]` em família `ia`
  ...
  de página (D68)
- **A pergunta da D18** tem dois botões `secundario`, do mesmo tamanho. Nenhum dos dois é o
  "certo"
...
No computador, duas colunas: a lista de threads (320 px) e a thread aberta. No celular, a lista
e depois a thread, com voltar.

```
┌ Seu time ─────────────────────┬ ▣ Corretor · faz e avisa; nota só com você ⓘ ────────┐
│ ▣ Corretor              (1)   │                                                      │
│   Corrigi as 32 provas do 2ºB │  ┌ Entrega · hoje, 9h50 ──────── [ Esperando você ] ┐│
│ ▣ Tutor                 (2)   │  │ Corrigi as 32 provas de estequiometria do 2ºB.   ││
│   8 alunos travaram em mol    │  │ Média 6,4. 5 casos fora da curva para abrir.     ││
│ ▣ Planejador                  │  │                      [ Ver resumo ] [ Revisar → ]││
│   Hoje: 3 aulas, 1 entrega…   │  └──────────────────────────────────────────────────┘│
│ ▣ Adaptador                   │                                                      │
└───────────────────────────────┴──────────────────────────────────────────────────────┘
```

- Cada linha: avatar, função, última frase, contador. Quem espera o professor sobe para o topo
- **O cabeçalho da thread diz a autonomia em português comum** (D9), com o ⓘ abrindo o que o
  agente faz sozinho, o que espera aprovação e o que nunca faz
...
- A thread do Tutor mostra **sinal, não conversa** (regra 50, item 10): o nome do aluno aparece
  só para o professor da turma (D34), e abrir a conversa é ação à parte, com aviso de que fica em
  auditoria
...
│  32 provas    média 6,4    ▁▂▅▇▅▂▁ distribuição    corrigido pelo Corretor [ IA ]    │
│                                                                                      │
...
│  ( tabela: quando · agente · o quê · turma · estado · aprovado por )                 │
│                                                                                      │
│  Uso por série e disciplina                ( barras, em agregado )                   │
└──────────────────────────────────────────────────────────────────────────────────────┘
```

- Quatro números em Fustat, `tabular-nums`, com rótulo em cima. Sem gráfico onde um número basta
...
- **Agentes** é uma grade de seis cartões, um por agente: avatar de 48 px, função, e três
  listas — *faz sozinho*, *espera aprovação*, *nunca faz* (D9, D60)

### 11.8 Entrada

Tela dividida, como a do Mulerun, mas página inteira e não janela:

- **Esquerda (metade, some abaixo de 768 px e vira faixa de 96 px):** fundo `noite`, a roseta em
  padrão estático bem apagado, a marca em negativo e uma frase da landing page. Ilustração em
  aberto (seção 7)
...
O roteiro do `ROADMAP.md` (D71), passo a passo, com a tela e onde ela está desenhada. **Nada
aqui começa antes de o F1 fechar e de esta Parte B estar aprovada.**

| Passo do roteiro | Tela | Desenho | Spec |
|---|---|---|---|
| Os três papéis entram e veem a própria navegação, com a marca | Entrada e casca | 11.8, 11.1 | A1 |
| 1. Coordenação mostra a escola montada e o material ingerido | Estrutura e Material, só leitura no seed | 11.1 | A1, A2 |
...
| 2. Em "Seu time", o Planejador abriu o dia | Seu time, e a pílula da Home | 11.4, 11.2 | A2 |
| 3. Aluno responde a atividade e abre o Tutor, que recusa, conduz e cita | Área do aluno | 11.6 | A3, A4 |
| 4. Professora recebe do Tutor que oito travaram; o Corretor espera | Seu time | 11.4 | A3, A4 |
| 4. Abre os destaques e aprova, com o registro da validação | Aprovar | 11.5 | A3 |
| 4. Vê o acerto por habilidade | Meu painel · Minhas turmas (barras) | 10.2 | A3 |
| 5. Coordenação abre a governança, os agentes e o resumo do Analista | Governança e Agentes | 11.7 | A5 |

Fora do MVP, com o lugar reservado no desenho: Calendário, Buscar, anexo e busca na web na caixa
de pedido, gráfico de série, modo sala, prova online, Conformidade, Denúncias, Exportar.
