...
| Convite de operador Turmma (hash do token, datas) | nossa equipe | primeiro acesso ao painel (A0) | execução do contrato de trabalho | 30 dias após usar, revogar ou vencer |
| Sessão de operador Turmma (horários de início, uso e fim, hash do cookie de renovação; sem IP) | nossa equipe | manter e encerrar o acesso ao painel: 8 h, ou 30 min sem uso (A0) | execução do contrato de trabalho; segurança (art. 46) | 30 dias após encerrar |
| Registro da operação (entrada, falha de entrada, saída, segundo fator configurado, operador criado ou desativado, com IP e data) | nossa equipe | segurança do painel e registro de acesso | obrigação legal (Marco Civil, art. 15); segurança | 6 meses, apagado pelo `sistema.expurgar-acesso`; a escola nunca lê |
