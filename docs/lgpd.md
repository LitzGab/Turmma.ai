...
| Convite de coordenador (hash do token, datas) | coordenador | primeiro acesso (F1), gerado pelo comando do operador ou pelo painel da operação (A0b) | execução de contrato | 30 dias após usar, revogar ou expirar, o que vier primeiro; apagado pelo `sistema.expurgar-acesso` (17.0), e junto com o usuário na eliminação |
...
| Auditoria da operação (autor, ação — operador criado ou desativado, segundo fator configurado, convite de operador gerado ou revogado —, operador alvo, data) | nossa equipe | prestação de contas sobre quem teve acesso ao painel, e quando | legítimo interesse; segurança (art. 46) | vigência + 5 anos, fora do expurgo de acesso |
