...
COPY apps/despachante apps/despachante
RUN npm run build -w @educa/shared && npm run build -w @educa/nucleo && npm run build -w @educa/despachante
...
COPY apps/web/package.json apps/web/
COPY apps/worker/package.json apps/worker/
RUN npm ci --omit=dev --ignore-scripts --workspace @educa/despachante && npm cache clean --force
...
COPY --from=construcao /repo/apps/despachante/dist apps/despachante/dist
USER node
WORKDIR /repo/apps/despachante
