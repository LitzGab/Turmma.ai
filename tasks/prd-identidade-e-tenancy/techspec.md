...
- **Concorrência:** `LOGIN_HASH_CONCORRENCIA` é obrigatório e vai no máximo até `UV_THREADPOOL_SIZE − 2`, conferido no boot.
...
| 10 | seção 6, FKs compostas, contexto de escola sem usuário | `conta` sem escola; quinze métodos `@SemEscopo` na fronteira de resolução; `registro_acesso` com escola nula na falha por e-mail |
