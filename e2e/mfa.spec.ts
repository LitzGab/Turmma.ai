...
import { codigoDoAutenticador, criarEquipeComSenha, type EquipeDeTeste } from './__fixtures__/sessao.ts'
...
const campoCodigo = (page: Page) => page.getByLabel(/Código do aplicativo|Digite o código que o aplicativo mostra/)

/** O segundo fator e os códigos de recuperação que a configuração entregou uma vez só. */
interface SegundoFatorConfigurado {
  readonly segredo: string
  readonly codigosDeRecuperacao: readonly string[]
}

async function entrarComSenha(page: Page, equipe: EquipeDeTeste): Promise<void> {
 ...
 */
async function configurarSegundoFator(page: Page, equipe: EquipeDeTeste): Promise<SegundoFatorConfigurado> {
